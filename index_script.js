...
firebase.auth().onAuthStateChanged(function(user) 
{
    if (user) 
    {
        ...
        .then(data => {
            const epochTime = parseInt(data) * 60 * 1000; // Convert minutes to seconds
...
function onClickEvent(button)
{
    ...
    var message = "Are you sure you want to book this " + selectedValue + " at " + readableTime(getMountainTime(parseInt(button.id))) + "?";
  var confirmationWindow = window.open("", "Confirmation", "width=300,height=200");
  confirmationWindow.document.write("<p>" + message + "</p>");
  confirmationWindow.document.write("<button onclick=\"window.opener.bookingConfirmed()\">Confirm</button>");
